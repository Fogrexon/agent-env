 ...
 * observed-graph.json): writing them must not break `readRun` / `listRuns`.
 */
...
// --- intent.json / effective-graph / observed-graph ---------------------------
{
  const store = createRunHistoryStore({ baseDir });
  const writer = store.open({
  ...
  console.log('✓ intent.json / effective-graph / observed-graph snapshots');
}
